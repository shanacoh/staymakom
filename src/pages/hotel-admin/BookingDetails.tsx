...
import { ArrowLeft, Loader2, Calendar, Users, CreditCard, Building, Mail, AlertTriangle } from "lucide-react";
...
export default function HotelBookingDetails() {
  ...
  const getPaymentBadge = (status: string) => {
    const map: Record<string, { variant: "default" | "secondary" | "destructive" | "outline"; label: string }> = {
      ...
      refund_pending: { variant: "destructive", label: "Remboursement dû" },
  ...
  if (!booking) {
    return (
      <div className="p-8">
        <Button variant="ghost" onClick={() => navigate("/hotel-admin/bookings")} className="mb-6">
  ...
  return (
    <div className="p-8 space-y-6 max-w-3xl">
      <Button variant="ghost" onClick={() => navigate("/hotel-admin/bookings")} className="mb-2">
        <ArrowLeft className="h-4 w-4 mr-2" />
        Retour aux réservations
      </Button>
      ...
      {booking.payment_status === "refund_pending" && (
        <div className="rounded-lg border border-destructive bg-destructive/10 p-4">
          <div className="flex items-center gap-2 text-destructive font-semibold">
            ...
            Remboursement client requis
          </div>
          {booking.refund_amount > 0 ? (
            <p className="text-sm mt-1">
              Montant : <span className="font-bold">{booking.refund_amount} {booking.currency}</span>
            </p>
          ) : (
            <p className="text-sm text-muted-foreground mt-1">Montant à vérifier directement avec le client.</p>
          )}
          {booking.customer_email && (
            <a href={`mailto:${booking.customer_email}`} className="text-sm text-destructive underline flex items-center gap-1 mt-1">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <Building className="h-4 w-4" /> Référence HyperGuest
          </CardTitle>
        </CardHeader>
        <CardContent>
